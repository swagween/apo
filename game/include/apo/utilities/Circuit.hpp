...
class Circuit {
  public:
	explicit Circuit(int const order, int const selection = 0) : m_order(order), m_selection(selection) { assert(order > 0 && selection >= 0); }
	void modulate(int const amount) { m_selection = (m_selection + m_order + amount) % m_order; }
